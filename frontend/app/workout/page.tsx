...
type Feedback = { rpe: number; soreness: number; pain: number; fatigue: number; enjoyment: number };
...
export default function WorkoutPage() {
  ...
  const [feedback, setFeedback] = useState<Feedback>({ rpe: 7, soreness: 2, pain: 0, fatigue: 3, enjoyment: 7 });
  ...
  const statusLabel = minimum ? "MINIMUM" : workout.kind === "RECOVERY" ? "RECOVERY" : "FULL";
  return (
    <AppShell>
      <div className="workout-page">
        ...
        <div className="workout-layout">
          <section className="workout-list">
            ...
            {mode === "manual" ? blocks.map((block, index) => <div className="workout-block" key={`${block.exercise_id}-${index}`}><span className="block-marker">0{index + 1}</span><div><div className="block-name">{block.name}</div><div className="block-intent">{block.intent}</div></div><span className="block-dose">{block.sets} × {block.reps ?? `${block.duration_seconds}s`}</span></div>) : <CameraPanel />}
            <div className="workout-buttons"><button className="button button-primary" onClick={() => setRunning((value) => !value)}>{running ? "PAUSE TIMER" : "START TIMER"}<span>{running ? "Ⅱ" : "▶"}</span></button><button className="button button-secondary" onClick={() => setMinimum((value) => !value)}>{minimum ? "USE FULL PLAN" : "SWITCH TO MINIMUM"}</button><button className="button button-secondary" onClick={finishSession}>{done ? "FEEDBACK BELOW" : "FINISH SESSION"}</button></div>
          </section>
        ...
        {done && <section className="feedback-card"><p className="eyebrow">Session closed</p><h2>Good. Now tell the system what it should learn.</h2><div className="form-grid"><div className="field"><label>Session RPE / {feedback.rpe}</label><input type="range" min="0" max="10" value={feedback.rpe} onChange={(e) => setFeedback({ ...feedback, rpe: Number(e.target.value) })} /></div><div className="field"><label>Soreness / {feedback.soreness}</label><input type="range" min="0" max="10" value={feedback.soreness} onChange={(e) => setFeedback({ ...feedback, soreness: Number(e.target.value) })} /></div><div className="field"><label>Pain / {feedback.pain}</label><input type="range" min="0" max="10" value={feedback.pain} onChange={(e) => setFeedback({ ...feedback, pain: Number(e.target.value) })} /></div><div className="field"><label>Enjoyment / {feedback.enjoyment}</label><input type="range" min="0" max="10" value={feedback.enjoyment} onChange={(e) => setFeedback({ ...feedback, enjoyment: Number(e.target.value) })} /></div></div><div className="flow-actions"><span className={error ? "error-message" : "field-help"}>{error || (recorded ? `Recorded as ${statusLabel}. Recovery can protect the next session.` : "These values influence the next plan.")}</span>{!recorded && <button className="button button-primary" onClick={recordFeedback}>SAVE FEEDBACK ↗</button>}</div></section>}
      </div>
    </AppShell>
  );
}
